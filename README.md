...
## 8. Почему тесты написаны именно так (примечание для проверяющего)
...
## 9. Дисклеймер о среде, в которой готовился проект

Код был подготовлен и многократно вычитан вручную в изолированной среде без доступа к Maven Central и без установленного браузера, поэтому **реальный прогон `mvn test` для получения скриншота Allure-отчёта в этой среде выполнить не удалось** — отчёт нужно сгенерировать самостоятельно по инструкции выше (п. 3–5) или дождаться прогона в GitHub Actions (п. 6). Логика каждого теста сверена построчно с исходным кодом сайта, чтобы свести риск ошибок к минимуму.
